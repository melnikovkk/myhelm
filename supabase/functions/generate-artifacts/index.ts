...
serve(async (req) => {
  ...
  try {
    const { prompt, locale } = await req.json() as ArtifactRequest;
    ...
    console.log('Generating artifacts for prompt:', prompt.substring(0, 100));
...
Верни ТОЛЬКО валидный JSON (без markdown, без \`\`\`) в следующем формате:
{
  ...
  "packages": [
    {"name": "Базовый", "price": "€XX", "description": "Краткое описание"},
    {"name": "Стандарт", "price": "€XX", "description": "Краткое описание"},
    {"name": "Премиум", "price": "€XX", "description": "Краткое описание"}
  ],
...
Return ONLY valid JSON (no markdown, no \`\`\`) in this format:
{
  ...
  "packages": [
    {"name": "Basic", "price": "€XX", "description": "Brief description"},
    {"name": "Standard", "price": "€XX", "description": "Brief description"},
    {"name": "Premium", "price": "€XX", "description": "Brief description"}
  ],
  ...
  "kpis": ["KPI 1", "KPI 2", "KPI 3"]
    ...
    try {
      ...
      console.log('Successfully parsed artifacts for:', artifacts.name);
